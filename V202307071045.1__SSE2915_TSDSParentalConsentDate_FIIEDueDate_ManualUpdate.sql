...
--------------- WithOUT FIIE Locked form -- ----------------
  ...
  select Ind11_TSDSParentConsentDate, ConsentDate_FromForm, ActualFIIEDate
from Compliances_ManualUpdate m 
left  join FIIELockedForm f on f.studentid = m.StudentId
where ECIReferral != 'Yes' 
and m.studentid = '019098'


select top 10 * from Compliances_ManualUpdate
...
FOR  
 ...
 where currentDistrictId <> 267 and (Age >= 3 or Age_FromPC >= 3) 
 and f.ActualFIIEDate is null
  and e.studentid = '019098'
...
begin tran
	...
	-- get current district for student one by one 
		...
		declare @initialConsentDate datetime =(Select top 1 Indicator11FIIEDueDate
												from Compliances_ManualUpdate e
												 left join FIIELockedForm f on e.Student_Uid = f.StudentUid
												 where currentDistrictId = @currentDistrict and (Age >= 3 or Age_FromPC >=3)
		...
		--	update  #student set Indicator11FIIEDueDate=@AdjustDay where RowNumber=@count and @SchooldaysMoved<=100
			update  s 
			set  SchooldaysMoved = @SchooldaysMoved , Ind11_IEPMeetingDueDate = @AdjustDay
			from Compliances_ManualUpdate s 			 
			where  Student_Uid = @student_uid and @SchooldaysMoved<=100 and (Age >= 3 or Age_FromPC >= 3)
