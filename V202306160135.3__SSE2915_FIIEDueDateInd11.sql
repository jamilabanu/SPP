...
BEGIN  
		...
		declare @currentDistrict int = (Select top 1 currentDistrictId from #tmp where RowNumber=@rowId)
		declare @initialConsentDate datetime =(Select top 1 InitialConsentForEvaluation from #tmp where RowNumber=@rowId)
		...
		declare @absentDay int = (select NumberOfAbsences from #tmp where RowNumber=@rowId and NumberOfAbsences>2)
...
END				
	...
	select c.Indicator12FIIEDueDate,  t.AdjustDay, t.SchooldaysMoved
	from Compliances  c inner join #tmp t on t.student_uid = c.student_uid
	where t.SchooldaysMoved < 100 and ECIReferral = 'Yes'

	--update c
	--set Indicator12FIIEDueDate = t.AdjustDay
	--from Compliances  c inner join #tmp t on t.student_uid = c.student_uid
	--where t.SchooldaysMoved < 100
	-- and ECIReferral = 'Yes'
