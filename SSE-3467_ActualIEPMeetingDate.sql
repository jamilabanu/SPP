...
*/
drop table BriefIEP_SSE_3467_ALL
drop table IEP_SSE_3467_ALL
...
f.d.value('(MeetsEvaluationCriteria/text())[1]', 'varchar(max)') as MeetsEvaluationCriteria,
     Fi.StudentUid 
   into BriefIEP_Eligibility_SSE_3467_ALL
from FormInstances fi 
outer apply fi.Data.nodes('Section2') f(d)
inner join BriefIEP_SSE_3467_ALL b on b.FormInstanceId = fi.ParentInstanceId --Section 2 from parent Brief
inner join Students s (nolock) on s.[Uid] = fi.[StudentUid]
inner join FormTypes ft on ft.Id=fi.FormTypeId
inner join Enrollments e on fi.[StudentUid] = e.StudentUid and e.ActiveRecord = 1
inner join Districts d on e.currentDistrictId = d.Id
...
drop table IEP_Eligibility_SSE_3467_ALL

select fi.id as FormInstanceId, fi.ParentInstanceId,
f.d.value('(MeetsEvaluationCriteria/text())[1]', 'varchar(max)') as MeetsEvaluationCriteria,
     Fi.StudentUid 
   into IEP_Eligibility_SSE_3467_ALL
from FormInstances fi 
outer apply fi.Data.nodes('Section2') f(d)
inner join IEP_SSE_3467_ALL b on b.FormInstanceId = fi.ParentInstanceId --Section 2 from parent IEP
inner join Students s (nolock) on s.[Uid] = fi.[StudentUid]
inner join FormTypes ft on ft.Id=fi.FormTypeId
inner join Enrollments e on fi.[StudentUid] = e.StudentUid and e.ActiveRecord = 1
inner join Districts d on e.currentDistrictId = d.Id
...
and b.RowNumber = 1
 ...
 from FIIELockedForm_AllForms_SSE_3467  f
 inner join BriefIEP_SSE_3467_ALL b on b.StudentUid = f.StudentUid
 ...
 from FIIELockedForm_AllForms_SSE_3467  f
 inner join IEP_SSE_3467_ALL b on b.StudentUid = f.StudentUid
 left join Compliances c on c.Student_Uid = f.studentuid
 where f.FIIE_RowNumber = 1
 ...
 select * from FIIELockedForm_AllForms_SSE_3467

 select  f.studentid, f.ActualFIIEDate, BriefIEPDate, ECIReferral, c.ActualIEPMeetingDate , be.MeetsEvaluationCriteria
 from FIIELockedForm_AllForms_SSE_3467  f
 inner join BriefIEP_SSE_3467_ALL b on b.StudentUid = f.StudentUid
 inner join BriefIEP_Eligibility_SSE_3467 be on be.parentinstanceid = b.formInstanceId
 left join Compliances c on c.Student_Uid = f.studentuid
 where f.FIIE_RowNumber = 1
 and ECIReferral = 'Yes'
 ...
 from FIIELockedForm_AllForms_SSE_3467  f
 inner join IEP_SSE_3467_ALL b on b.StudentUid = f.StudentUid
 inner join IEP_Eligibility_SSE_3467 be on be.parentinstanceid = b.formInstanceId
 left join Compliances c on c.Student_Uid = f.studentuid
 where f.FIIE_RowNumber = 1
 and ECIReferral = 'Yes'
  ...
  -- SPP 11 --
  drop table ActualIEP_SSE_3467_ALL

  select * from FIIELockedForm_AllForms_SSE_3467_ALL
 ...
 into ActualIEP_SSE_3467_ALL
 from FIIELockedForm_AllForms_SSE_3467_ALL  f
 inner join BriefIEP_SSE_3467_ALL b on b.StudentUid = f.StudentUid
 left join Compliances c on c.Student_Uid = f.studentuid
 where f.FIIE_RowNumber = 1
 ...
 inner join IEP_SSE_3467_ALL b on b.StudentUid = f.StudentUid
 left join Compliances c on c.Student_Uid = f.studentuid
 where f.FIIE_RowNumber = 1 and b.RowNumber = 1
 ...
 drop table ActualIEP_AllForms_SSE_3467

 select row_number() over(partition by  Studentid order by IEPDate desc) as RowNumber,* 
 into ActualIEP_AllForms_SSE_3467_ALL
 from ActualIEP_SSE_3467_ALL
 -- where studentid = '100509'

 select * from ActualIEP_AllForms_SSE_3467
 where RowNumber = 1 
 and (Indicator11ActualIEPMeetingDate IS NULL or IEPDate <> Indicator11ActualIEPMeetingDate)

 select * from ActualIEP_SSE_3467_ALL
 where studentid = '141598'

 drop table FIIELockedForm_SSE_3467

 select   row_number() over(partition by  fi.StudentUid order by fi.datelocked desc) as FIIE_RowNumber, DateLocked, d.name as districName,CurrentDistrictId as DistrictId ,DateOfBirth ,studentid ,Firstname ,lastname , 
		   f.d.value('(Date/text())[1]', 'varchar(max)')   as [ActualFIIEDate], Fi.StudentUid
into FIIELockedForm_SSE_3467_ALL
from FormInstances fi 
outer apply fi.Data.nodes('Title') f(d)
left join Students s (nolock) on s.[Uid] = fi.[StudentUid]
left join Compliances c on c.Student_Uid=fi.StudentUid 	
...
from ActualIEP_AllForms_SSE_3467 f
 ...
 select * from ActualIEP_AllForms_SSE_3467 where studentid = '102574'
 ...
 drop table FIIELockedForm_AllForms_SSE_3467_ALL

 select * from FIIELockedForm_AllForms_SSE_3467

  select  * 
 into FIIELockedForm_AllForms_SSE_3467_ALL
 from FIIELockedForm_SSE_3467_ALL
...
2023-04-11 00:00:00.000
 ...
 select * from ActualIEP_AllForms_SSE_3467_ALL
