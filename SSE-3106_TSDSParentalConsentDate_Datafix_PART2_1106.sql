...
drop table TSDS_3106_S1_P2 
select c.Student_Uid, s.studentid, e.CurrentDistrictId,  InitialConsentForEvaluation,   
...
into TSDS_3106_S1_P2
from Compliances c 
inner join Enrollments e on e.StudentUid = c.Student_Uid and ActiveRecord = 1
inner join students s on s.uId = c.Student_Uid
where  InitialConsentForEvaluation >= '01/01/2023'  --and s.StudentId = 'T01212021BA'
--and ReferralType = 'Yes' 
and isnull(c.Indicator11TSDSParentalConsentDate, '') != (select [dbo].[fnAdjustSchoolDays_TSDSParentConsentDate](currentDistrictId, InitialConsentForEvaluation, null, 1))

--select * from districtdates where districtid = 698

select * from TSDS_3106_S1_p2 where studentid = '076184'
...
into TSDS_3106_S2_P2
from Compliances c 
inner join Enrollments e on e.StudentUid = c.Student_Uid and ActiveRecord = 1
inner join tmp_3106_sept_p2 t on t.Student_Uid = c.Student_Uid
inner join ConsentForEval_3107_Sept s on s.studentuid = t.Student_Uid
inner join NoticeOfEval_p2 n on n.StudentUid = t.Student_Uid
where InitialConsentForEvaluation >= '01/01/2023'
...
and isnull(c.Indicator11TSDSParentalConsentDate,'') != (select [dbo].[fnAdjustSchoolDays_TSDSParentConsentDate](t.currentDistrictId,t.ParentSignatureDate, c.Indicator11ActualFIIEDate, 1))
...
-- send to Steph--
-- Scenario 2:
select * from TSDS_3106_S2_P2 p
inner join compliances c on c.Student_Uid = p.Student_Uid
where c.Indicator11TSDSParentalConsentDate is null 

where studentid = 'T01212021BA'

select * from ConsentForEval_3107_Sept
where studentid = '8282019'

-- Scenario 1:
select  p.* from TSDS_3106_S1_p2 p
inner join compliances c on c.Student_Uid = p.Student_Uid
where p.student_uid not in (select distinct student_uid from TSDS_3106_S2_p2)
and c.Indicator11TSDSParentalConsentDate is null 
--and p.StudentId = '473854'
